...
class PriorityCache:
    ...
    def query(self, queries, topk):
        ...
        if queries.device.type == 'cpu' and queries.dtype == torch.float16:
            dist = torch.cdist(queries.float(), supports.float()).to(self.dtype)
        else:
    ...
    def retained_bytes(self):
        """Return bytes in entries currently retained by this cache."""
        retained_tensors = (
            self.keys[:self.size],
...
class Ramen(TTABase):
    ...
    def memory_bytes(self):
        """Return bytes in support entries currently retained across caches."""
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            ...
            # Legacy Ramen updates a whole batch before retrieval, so this
            # scalar represents the batch-final retained state for every
            # sample emitted by this forward call.
